...
void cut_voxel(unordered_map<VOXEL_LOC, OCTO_TREE *> &feat_map, pcl::PointCloud<PointType>::Ptr pl_feat, Eigen::Matrix3d R_p, Eigen::Vector3d t_p, int feattype, int fnum, int capacity)
{
    ...
    for (uint i = 0; i < plsize; i++)
    {
        ...
        if (iter != feat_map.end())
        {
            ...
            iter->second->is2opt = true;    // doc: 标记该voxel需要进行优化
        }
        else // If not finding, build a new voxel   // doc: 因为新建的voxel里面本身就没有原先 map 的点，所以没有进行优化，is2opt = false
        {
...
int main(int argc, char **argv)
{
    ...
    // doc: 一开始一般需要静止 accumulate_window = 3 帧左右，因为需要累计多一点点再进行配准
    while (n.ok())
    {
        ros::spinOnce();

        // The thread of map_refine is done
        // doc: 如果 map_refine 线程完成了，对位姿进行更新
        if (opt_lsv.read_refine_state() == 2)   // 初始时为 0
        {
            ...
            for (int i = 0; i < margi_size; i += pub_skip)
            {
                ...
                pcl::transformPointCloud(*pl_full_buf[window_base + i], pcloud, trans); // doc: pl_full_buf 是一个存储lidar系点云指针的vector数组，margin的点云帧会释放内存，不用当心占内存

                pl_send += pcloud;
            }
            pub_func(pl_send, pub_full, ct);    // doc：发布点云

            for (int i = 0; i < margi_size; i++)
            {
                pl_full_buf[window_base + i] = nullptr; // doc: 释放 margin 的点云，智能指针计数为0时会自动释放
            }
            ...
            pl_surf_centor_map.clear(); // doc: pl_surf_centor_map 存储的是所有体素的中心点以及法相量
            ...
            for (auto iter = surf_map.begin(); iter != surf_map.end(); ++iter)
            {
                ...
                // doc: 因为 marginalize 会调整地图，所以需要重新构建地图的平面信息点云（点 + normal）
            ...
            // doc: window size of every voxel
        ...
        mBuf.unlock();
        plcount++;  // doc: 记录从开始到现在，拿到多少帧点云
        // doc: The number of scans in the sliding window
        OCTO_TREE::voxel_windowsize = plcount - window_base;    // doc: 减去窗口内第一帧，自然就得到窗口内帧数
        ...
        // Scan2map module
        // doc: 累积 accumulate_window = 3 帧之后开始 Scan2map 配准
        if (plcount > accumulate_window)    // doc: accumulate_window = 3，对于面
        {
            ...
            if (plcount <= scan2map_on) // doc: scan2map_on = 10，初始化时需要存储几帧之后才开始，所以 pl_surf_fil_map 只会在开始的时候用上，之后都是走 else 分支
            {
            ...
            for (int itercount = 0; itercount < 2; itercount++)
            {
                ...
                a_size = pl_surf->size();
                if (plcount <= scan2map_on) // ?: if 分支比较难理解，涉及到初始化的一些操作，else 分支比较容易理解
                {
                ...
                else
                {
                    ...
                    for (uint i = 0; i < a_size; i++)
                    {
                        ...
                        for (int j = 0; j < ns; j++)
                        {
                            ...
                            double dista = fabs(direct.dot(aft_tran - center)); // doc: 点到面距离
                        ...
                        // doc: 找到最近的面，将当前帧的点 p_orig、最近面中心 kervec、法相量 orient、距离 range 送入LM优化器
                        sld.push_surf(p_orig, kervec, orient, (1 - 0.75 * range));
                    }
        ...
        // doc: 累计一下初始地图，plcount 超过 scan2map_on 之后就不需要 pl_surf_fil_map 了
        if (plcount <= scan2map_on) // doc: scan2map_on = 10
        {
        ...
        if (plcount - window_base - window_size > 10)   // doc: window_size = 20
        {
            printf("Out of size\n");
            exit(0);
        }
        ...
        // doc: 把最新帧放进地图，这也是为什么后面循环主体开始的时候需要进行 margin，并且标记后面需要进行 refine 的第零层 voxel，i.e. is2opt = true
        ...
        // doc: Points in new scan have been distributed in corresponding root node voxel
        // doc: Then continue to cut the root voxel until right size
        // doc: 递归构建
        for (auto iter = surf_map.begin(); iter != surf_map.end(); ++iter)
        {
        ...
        // doc: Begin map refine module
        // doc: 超过 window_size = 20 之后才开始 refine map
        if (plcount >= window_base + window_size && opt_lsv.read_refine_state() == 0)   // doc: window_size = 20
        {
            for (int i = 0; i < window_size; i++)
            {
            ...
            else
            {
                ...
                for (auto iter = surf_map.begin(); iter != surf_map.end(); ++iter)
                {
                    if (iter->second->is2opt)
                    {
                        iter->second->traversal_opt(opt_lsv);   // doc: Push voxel into "opt_lsv" (LM optimizer)
                    }
                }
