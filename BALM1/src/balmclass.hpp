...
public:
    ...
    void push_voxel(vector<vector<Eigen::Vector3d> *> &plvec_orig, SIG_VEC_CLASS &sig_vec, int lam_type)
    {
        ...
        for (int i = 0; i < slwd_size; i++) // doc: sliding window size
        {
        ...
        // Only one scan    // doc: 意味着落在这个 voxel 内的帧数小于 1，独苗没有优化的必要
        if (process_points_size <= 1)
        {
            return;
        }

        int filternum2use = filternum;  // doc: = 1
        ...
        plvec_voxel->reserve(filternum2use * slwd_size);    // doc: 存储点
        slwd_num->reserve(filternum2use * slwd_size);       // doc: 存储点所对应的窗口内的哪一帧

        // retain one point for one scan (you can modify)
        // doc: 降采样，根据 filternum2use 等分
        ...
        plvec_voxels.push_back(plvec_voxel); // doc: Push a voxel into optimizer
        slwd_nums.push_back(slwd_num);  // doc: 存储 plvec_voxels 中每个点所对应的窗口内的哪一帧
        lam_types.push_back(lam_type);  // doc: 0 is surf, 1 is line
        sig_vecs.push_back(sig_vec); // doc: history points out of sliding window
    }
    ...
    void acc_t_evaluate(vector<SO3> &so3_ps, vector<Eigen::Vector3d> &t_ps, int head, int end, Eigen::MatrixXd &Hess, Eigen::VectorXd &JacT, double &residual)
    {
        ...
        for (int a = head; a < end; a++)
        {
            ...
            vector<int> &slwd_num = *slwd_nums[a];  // doc: 该 voxel 中每个点所对应的窗口内的哪一帧
            uint backnum = plvec_voxel.size();  // doc: 该 voxel 中点的个数

            Eigen::Vector3d vec_tran;
            vector<Eigen::Vector3d> plvec_back(backnum);
            // derivative point to T (R, t) 3×6
            ...
            for (uint i = 0; i < backnum; i++)
            {
                ...
                vec_Jt = 2.0 / N_points * ukukT * plvec_back[i];    // doc: 公式(6)：lamda_k 相对于 p_i 的导数
                _jact.block<3, 1>(6 * slwd_num[i] + 3, 0) += vec_Jt;    // doc: 公式(10)
                _jact.block<3, 1>(6 * slwd_num[i], 0) -= point_xis[i] * vec_Jt; // doc: 公式(10)
            }

            // doc: Hessian matrix 对照论文吧hh
    ...
    void damping_iter()
    {
        ...
        for (int i = 0; i < iter_max; i++)
        {
            if (is_calc_hess)
            {
                // doc: calculate Hessian, Jacobian, residual
...
public:
    ...
    void recut(int layer, uint frame_head, pcl::PointCloud<PointType> &pl_feat_map)
    {
        ...
        // doc: 这里的意思是，如果是第0层，也就是root层，会把点都保留，但如果不是的话，就会把点清空，因为已经给子节点了，就不要重复存储了
        if (layer != 0)
        {
            for (int i = frame_head; i < OCTO_TREE::voxel_windowsize; i++)
            {
    ...
    // doc: marginalize 5 scans in slidingwindow (assume margi_size is 5)
    void marginalize(int layer, int margi_size, vector<Eigen::Quaterniond> &q_poses, vector<Eigen::Vector3d> &t_poses, int window_base, pcl::PointCloud<PointType> &pl_feat_map)
    {
        ...
        {
            if (octo_state != 1) // doc: 如果是叶子节点，就需要更新全局点的坐标
            {
                for (int i = 0; i < OCTO_TREE::voxel_windowsize; i++) // doc: voxel_windowsize 窗口内帧数
                {
                    // Update points by new poses
                    // doc: 更新 voxel 内 world 系下的点
            ...
            // doc: 把 margin 掉的点固定，并更新对应的协方差和均值
            uint a_size;
            if (feat_eigen_ratio > feat_eigen_limit[ftype])
            {
            ...
            // doc: Clear front 5 scans 把前五帧的点清空，但是 i 所对应位置还是在的
            for (int i = 0; i < margi_size; i++)
            {
            ...
            // doc: 把前面清空的5帧挪到后面，由以后新的帧來覆盖
            // doc: 因此，如果没有新的点进来的话，就会逐渐变空
            for (int i = margi_size; i < OCTO_TREE::voxel_windowsize; i++)
            {
            ...
            if (octo_state != 1) // doc: octo_state = 0 表示平面或者叶子节点
            {
                points_size = 0;
                ...
                if (feat_eigen_ratio >= feat_eigen_limit[ftype])
                {
                    pl_feat_map.push_back(ap_centor_direct);    // doc: 更新根节点的平面点云
                }
            }
        }
    ...
    void traversal_opt_calc_eigen()
    {
        ...
        Eigen::SelfAdjointEigenSolver<Eigen::Matrix3d> saes(covMat);
        feat_eigen_ratio_test = (saes.eigenvalues()[2] / saes.eigenvalues()[ftype]);    // doc: 2 是最大的
    }

    // Push voxel into "opt_lsv" (LM optimizer)
    void traversal_opt(LM_SLWD_VOXEL &opt_lsv)
    {
        if (octo_state != 1)    // doc: 叶子节点
        {
            sw_points_size = 0;
            for (int i = 0; i < OCTO_TREE::voxel_windowsize; i++)
            {
            ...
            if (feat_eigen_ratio_test > opt_feat_eigen_limit[ftype])    // doc: feat_eigen_ratio_test 越大越好
            {
                opt_lsv.push_voxel(plvec_orig, sig_vec, ftype); // doc: sig_vec: history points out of sliding window
            }
        }
        else
        {
...
public:
    ...
    void evaluate_para(SO3 &so3_p, Eigen::Vector3d &t_p, Eigen::Matrix<double, 6, 6> &Hess, Eigen::Matrix<double, 6, 1> &g, double &residual)
    {
        ...
        for (uint i = 0; i < a_size; i++)
        {
            ...
            residual += surf_coeffs[i] * d_vec.dot(d_vec); // doc: 残差 = 法相量 * 点到面距离 = n * d
            Hess += surf_coeffs[i] * jacob.transpose() * jacob;
            g += surf_coeffs[i] * jacob.transpose() * d_vec; // doc: 等价于对旋转和平移施加右扰动求导，注意：(Rp)^ = Rp^
        }
    ...
    void damping_iter()
    {
        ...
        for (int i = 0; i < 20; i++)
        {
            ...
            if (q > 0) // doc: LM法的 Nielson 策略
            {
                ...
                v = 2;
                q = 1 - pow(2 * q - 1, 3);
                u *= (q < one_three ? one_three : q); // doc: ... one_three = 1/3
            }
            else // doc: 残差反而变大的话，增大阻尼项，并且本轮位姿不更新
            {
                u = u * v;
                v = 2 * v;
            }
